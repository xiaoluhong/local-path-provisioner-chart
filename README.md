...
```console
cd local-path-provisioner-chart
helm install --name local-path-storage --namespace local-path-storage --set storageClass.provisionerName=rancher.io/local-path ./
```
...
```console
cd local-path-provisioner-chart
helm install --name local-path-storage --namespace local-path-storage ./ -f values.yaml
```
...
```console
cd local-path-provisioner-chart
helm install --name local-path-storage --namespace local-path-storage --set rbac.create=true ./
```
